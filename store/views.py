...
def checkout_paypal(request, cart, orders):
    if request.user.is_authenticated():
        ...
        paypalrestsdk.configure({
            "mode": "sandbox",
            "client_id": "AVlVtVBU2gIiCIyi39tDZ0tWbXf6pAYfB8Vgi6O1LDjIEaGl9GBV6Wt-2-_ssFQk4N9vY3BOEa0wD9om",
            "client_secret": "EN4V4cLqTSC_Il7qFegT7YPVQIoKBa6T7kKMQKZDKr-eo1tZyuGZlFwBlHC7CfcZgyvbVO1maufl-VSr"})
...
def checkout_stripe(cart, orders, token):
    stripe.api_key = "sk_test_nJUvtSUs8fUluxPl4lhFIOZq"
    total = 0
    for order in orders:
        total += (order.book.price * order.quantity)
    status = True
